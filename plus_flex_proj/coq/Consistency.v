...
Lemma consistent_mergel: forall v1 v2 v,
    consistent (e_merge v1 v2) v -> consistent v1 v /\ consistent v2 v.
Proof.
  intros v1 v2 v H.
  inductions H.
  -
...
Lemma consistent_merger: forall v1 v2 v,
    consistent v (e_merge v1 v2) -> consistent v v1 /\ consistent v v2.
Proof.
  intros v1 v2 v H.
  inductions H.
  -
...
Lemma consistent_top_l : forall u v,
    consistent u v -> consistent e_top v.
Proof.
  introv Hc.
  forwards Hp: consistent_prevalue2 Hc.
  forwards (?&?): prevalue_exists_ptype Hp.
  applys* C_disjoint.
Qed.

Lemma consistent_top_r : forall u v,
    consistent u v -> consistent u e_top.
Proof.
  introv Hc.
  forwards Hp: consistent_prevalue1 Hc.
  forwards (?&?): prevalue_exists_ptype Hp.
  applys* C_disjoint.
Qed.

Lemma consistent_meg_l : forall v u1 u2,
    consistent (e_merge u1 u2) v <-> consistent (meg_exp u1 u2) v.
Proof.
  split; introv Hc.
  - forwards* (?&?): consistent_mergel Hc.
    destruct u1; destruct u2; simpl; eauto.
  - destruct u1; destruct u2; simpl in Hc; eauto; constructor*.
    all: applys* consistent_top_l.
Qed.

Lemma consistent_meg_r : forall v u1 u2,
    consistent v (e_merge u1 u2) <-> consistent v (meg_exp u1 u2).
Proof.
  split; introv Hc.
  - forwards* (?&?): consistent_merger Hc.
    destruct u1; destruct u2; simpl; eauto.
  - destruct u1; destruct u2; simpl in Hc; eauto; constructor*.
    all: applys* consistent_top_r.
Qed.
