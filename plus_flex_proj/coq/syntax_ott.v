...
(* defns OriginalBCDDeclarativeSubtyping *)
Inductive original_bcd_sub : typ -> typ -> Prop :=    (* defn original_bcd_sub *)
 | OS_refl : forall (A:typ),
     original_bcd_sub A A
 | OS_trans : forall (A C B:typ),
     original_bcd_sub A B ->
     original_bcd_sub B C ->
     original_bcd_sub A C
 | OS_top : forall (A:typ),
     original_bcd_sub A t_top
 | OS_topArr : 
     original_bcd_sub t_top (t_arrow t_top t_top)
 | OS_topRcd : forall (l:i),
     original_bcd_sub t_top (t_rcd l t_top)
 | OS_arr : forall (A C B D:typ),
     original_bcd_sub B A ->
     original_bcd_sub C D ->
     original_bcd_sub (t_arrow A C) (t_arrow B D)
 | OS_and : forall (A B C:typ),
     original_bcd_sub A B ->
     original_bcd_sub A C ->
     original_bcd_sub A (t_and B C)
 | OS_andl : forall (A B:typ),
     original_bcd_sub (t_and A B) A
 | OS_andr : forall (A B:typ),
     original_bcd_sub (t_and A B) B
 | OS_distArr : forall (A B C:typ),
     original_bcd_sub (t_and  (t_arrow A B)   (t_arrow A C) ) (t_arrow A (t_and B C))
 | OS_rcd : forall (l:i) (A B:typ),
     original_bcd_sub A B ->
     original_bcd_sub (t_rcd l A) (t_rcd l B)
 | OS_distRcd : forall (l:i) (A B:typ),
     original_bcd_sub (t_and (t_rcd l A) (t_rcd l B)) (t_rcd l (t_and A B)).
...
Inductive msub : typ -> typ -> Prop :=    (* defn msub *)
 | MS_int : 
     msub t_int t_int
 | MS_top : forall (A B:typ),
     topLike B ->
     msub A B
 | MS_andl : forall (A B C:typ),
     msub A C ->
     msub (t_and A B) C
 | MS_andr : forall (A B C:typ),
     msub B C ->
     msub (t_and A B) C
 | MS_arr : forall (A C B D:typ),
     msub B A ->
     msub C D ->
     msub (t_arrow A C) (t_arrow B D)
 | MS_rcd : forall (l:i) (C D:typ),
     msub C D ->
     msub (t_rcd l C) (t_rcd l D)
 | MS_and : forall (A D B C:typ),
     spl D B C ->
     msub A B ->
     msub A C ->
     msub A D.
...
Inductive subsub : typ -> typ -> Prop :=    (* defn subsub *)
 ...
 | SubSub_top : forall (A:typ),
     topLike A ->
     subsub t_top A
 | SubSub_split : forall (A1 A2 B B1 B2:typ),
     spl B B1 B2 ->
     subsub A1 B1 ->
     subsub A2 B2 ->
     subsub (t_and A1 A2) B
 | SubSub_topL : forall (A B B1 B2:typ),
     spl B B1 B2 ->
     topLike B1 ->
     subsub A B2 ->
     subsub A B
 | SubSub_topR : forall (A B B1 B2:typ),
     spl B B1 B2 ->
     topLike B2 ->
     subsub A B1 ->
     subsub A B.
...
(* defns ConvertToRecordType *)
Inductive rcdTyp : i -> typ -> typ -> Prop :=    (* defn rcdTyp *)
 | RT_arr : forall (l:i) (A B:typ),
     rcdTyp l (t_arrow A B) t_top
 | RT_topRcd : forall (l:i),
     rcdTyp l t_top t_top
 | RT_rcd : forall (l:i) (A:typ),
     rcdTyp l (t_rcd l A) A
 | RT_rcdNeq : forall (l1 l2:i) (A:typ),
      l1  <>  l2  ->
     rcdTyp l1 (t_rcd l2 A) t_top
 | RT_and : forall (l:i) (A B A' B':typ),
     rcdTyp l A A' ->
     rcdTyp l B B' ->
     rcdTyp l (t_and A B)  (meg_typ  A'   B' ) .
...
Inductive papp : exp -> vl -> exp -> Prop :=    (* defn papp *)
 | Papp_app : forall (A:typ) (e:exp) (B:typ) (v v':exp),
     lc_exp (e_abs A e B) ->
     TypedReduce v A v' ->
     papp (e_abs A e B) (vl_exp v) (e_anno  (  (open_exp_wrt_exp  e v' )  )  B)
 | Papp_absProj : forall (A:typ) (e:exp) (B:typ) (l:i),
     lc_exp (e_abs A e B) ->
     papp (e_abs A e B) (vl_la l) e_top
 | Papp_rcdNeq : forall (l1:i) (v:exp) (l2:i),
     lc_exp v ->
      l1  <>  l2  ->
     papp (e_rcd l1 v) (vl_la l2) e_top
 ...
 | Papp_mergeProj : forall (v1 v2:exp) (l:i) (e1 e2:exp),
     papp v1 (vl_la l) e1 ->
     papp v2 (vl_la l) e2 ->
     papp (e_merge v1 v2) (vl_la l)  (meg_exp  e1   e2 ) 
 | Papp_mergeApp : forall (v1 v2 v e1 e2:exp),
     papp v1 (vl_exp v) e1 ->
     papp v2 (vl_exp v) e2 ->
     papp (e_merge v1 v2) (vl_exp v) (e_merge e1 e2).
...
(** infrastructure *)
Hint Constructors value topLike spl ord disjoint original_bcd_sub algo_sub msub subsub arrTyp rcdTyp TypedReduce papp step prevalue pType consistent Typing lc_exp lc_vl : core.
