...
Fixpoint erase_anno (e:exp) : dexp :=
  ...
  | e_fixpoint A e => de_fixpoint (erase_anno e)
  | e_app e1 e2 => de_app (erase_anno e1) (erase_anno e2)
  | e_merge e1 e2 => de_merge (erase_anno e1) (erase_anno e2)
  | e_pair e1 e2 => de_pair (erase_anno e1) (erase_anno e2)
  | e_projl e => (erase_anno e)
  | e_projr e => (erase_anno e)
...
Proof.
  ...
  induction H;
    simpl;
    try constructor*;
    try forwards*: erasure_lc H.
Qed.

Lemma D_value_lc : forall v,
    DValue v -> lc_dexp v.
Proof.
  intros v H.
  induction H;
...
Proof.
  ...
  - simpl.
    econstructor.
    apply DStep_toparr. lets* : erasure_val H.
    apply star_refl.
  -
    simpl.
  ...
  -
    ...
    apply star_onestep.
    assert (| e ^^ e_fixpoint A e | = open_dexp_wrt_dexp (| e |)  (| e_fixpoint A e |)) by apply erasure_open.
    simple. rewrite H0.
    apply~ DStep_fix.
Qed.
*)
